...
def page_indicateurs(df: pd.DataFrame):
    ...
    with tab_vue:
        ...
        # ── Graphique Actives vs Fermées — Top 20 communes ────────
        top20_ent = df_ent.nlargest(20,"nb_entreprises_total").copy()
        df_bar = pd.concat([
            top20_ent[["ville","nb_entreprises_actives"]].rename(columns={"nb_entreprises_actives":"Nb"}).assign(Type="Actives"),
            top20_ent[["ville","nb_fermees"]].rename(columns={"nb_fermees":"Nb"}).assign(Type="Fermées"),
        ])
        bar_ent = alt.Chart(df_bar).mark_bar(cornerRadiusTopLeft=3,cornerRadiusTopRight=3).encode(
            ...
            y=alt.Y("Nb:Q",
                axis=alt.Axis(labelColor="#8895AA",gridColor="#F0F4F8",
                              domainColor="transparent",labelFontSize=10,title="Nb entreprises")),
            color=alt.Color("Type:N",
                scale=alt.Scale(domain=["Actives","Fermées"],range=["#059669","#DC2626"]),
                legend=alt.Legend(orient="top-right",labelFontSize=10,title="")),
            xOffset="Type:N",
            tooltip=["ville:N","Type:N","Nb:Q"]
        ).properties(height=240,
            title=alt.TitleParams("Entreprises actives vs fermées · Top 20 communes",
            fontSize=13,fontWeight=700,color="#0A0F1E",subtitle="Fermées = cumulatif depuis création")
        ).configure_view(strokeWidth=0).configure_axis(labelFont="Plus Jakarta Sans")
        st.altair_chart(bar_ent, use_container_width=True)

        # ── Taux de survie par département (barres horizontales) ──
        df_surv = df[["ville","dept_nom","taux_survie_entreprises","nb_entreprises_actives"]].copy()
        df_surv["Survie %"] = (df_surv["taux_survie_entreprises"]*100).round(1)
        top_surv = df_surv.nlargest(15,"Survie %")
        bar_surv = alt.Chart(top_surv).mark_bar(cornerRadiusTopRight=4,cornerRadiusBottomRight=4).encode(
            y=alt.Y("ville:N", sort="-x",
                axis=alt.Axis(labelColor="#8895AA",labelFontSize=9,title="")),
            x=alt.X("Survie %:Q", scale=alt.Scale(domain=[60,100]),
                axis=alt.Axis(labelColor="#8895AA",gridColor="#F0F4F8",labelFontSize=10,title="Taux de survie (%)")),
            color=alt.Color("dept_nom:N",
                scale=alt.Scale(domain=["Essonne","Val-de-Marne"],range=["#1A56DB","#DC2626"]),
                legend=alt.Legend(orient="bottom-right",labelFontSize=10,title="")),
            tooltip=["ville:N","dept_nom:N","Survie %:Q","nb_entreprises_actives:Q"]
        ).properties(height=220,
            title=alt.TitleParams("Top 15 — Taux de survie des entreprises par commune",
            fontSize=13,fontWeight=700,color="#0A0F1E")
        ).configure_view(strokeWidth=0).configure_axis(labelFont="Plus Jakarta Sans")
        st.altair_chart(bar_surv, use_container_width=True)
