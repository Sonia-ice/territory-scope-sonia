...
def page_deserts(df: pd.DataFrame):
    ...
    tooltip_d = {
        "html": (
            ...
            "padding:10px 13px;border-radius:10px;border:1px solid #E2E8F2;"
            "box-shadow:0 4px 16px rgba(10,15,30,.12);min-width:160px;'>"
            "<b style='font-size:13px;color:#0A0F1E;'>{ville}</b><br>"
            "<span style='font-size:11px;color:#8895AA;'>{dept_nom}</span><br>"
            "<span style='font-size:11px;color:#DC2626;font-weight:600;'>{niveau}</span>"
            "</div>"
        ),
...
def page_communes(df: pd.DataFrame):
    ...
    with tab_comp:
        ...
        # Mini carte 2 communes — pydeck satellite
        try:
            ...
            df_2 = pd.DataFrame([
                {"latitude":lat_A,"longitude":lon_A,"ville":A,
                 "color":[26,86,219,230],"dept":str(rA["dept_nom"]),"score":f"{float(rA.get('score_attractivite',0)):.0%}"},
                {"latitude":lat_B,"longitude":lon_B,"ville":B,
                 "color":[109,40,217,230],"dept":str(rB["dept_nom"]),"score":f"{float(rB.get('score_attractivite',0)):.0%}"},
            ])
            ...
            st.pydeck_chart(pdk.Deck(
                layers=[
                    pdk.Layer("ColumnLayer", data=df_2,
                        get_position="[longitude,latitude]",
                        get_elevation=2000, elevation_scale=1,
                        radius=400, get_fill_color="color",
                        pickable=True, coverage=0.88, auto_highlight=True),
                    pdk.Layer("TextLayer", data=df_2,
                        get_position="[longitude,latitude]",
                        get_text="ville",
                        get_size=14, get_color=[255,255,255,240],
                ...
                initial_view_state=pdk.ViewState(
                    ...
                    zoom=zoom_lvl, pitch=45, bearing=0),
                tooltip={
                    "html": (
                        "<div style='font-family:sans-serif;background:#fff;"
                        "padding:10px 13px;border-radius:10px;border:1px solid #E2E8F2;'>"
                        "<b style='font-size:13px;'>{ville}</b><br>"
                        "<span style='color:#8895AA;font-size:11px;'>{dept}</span><br>"
                        "<span style='color:#1A56DB;font-weight:600;'>⭐ {score}</span>"
                        "</div>"
                    ),
                    "style": {"backgroundColor":"transparent","padding":"0"}
                },
            ...
            st.info(f"📍 **{A}** · {rA.get('dept_nom','')} &nbsp;|&nbsp; 📍 **{B}** · {rB.get('dept_nom','')} — coordonnées non disponibles pour ces communes.")
