...
def render_sidebar(df: pd.DataFrame):
    ...
    # Données pour les popovers
    df_prio   = df[df["cat_attractivite"] == "Zone Prioritaire"].nlargest(8, "score_attractivite")[
        ["ville","dept_nom","score_attractivite","taux_chomage","revenu_median"]
    ] if _prio > 0 else pd.DataFrame()

    df_sig = df[df["cat_signal_faible"] == "Signal Fort"].nlargest(8, "score_signal_faible")[
        ["ville","dept_nom","score_signal_faible","taux_chomage","taux_pauvrete"]
    ] if _sig > 0 else pd.DataFrame()

    df_des = df[df["cat_desert_medical"] == "Fort"].nlargest(8, "score_desert_medical")[
        ["ville","dept_nom","score_desert_medical","medecins_10k_hab","population"]
    ] if _desert > 0 else pd.DataFrame()
    ...
    with st.sidebar:
        ...
        st.markdown(
            '<p style="font-size:9px;font-weight:700;color:#94A3B8;text-transform:uppercase;'
            'letter-spacing:1.2px;margin:0 0 8px;padding:0 4px;">Vue d\'ensemble</p>',
            unsafe_allow_html=True
        )
        ...
        # Carte Communes (non cliquable, informatif)
        with ka:
            st.markdown(
                f"<div style='background:rgba(255,255,255,.05);border-radius:10px;padding:10px 12px;"
                f"text-align:center;'>"
                f"<div style='font-size:22px;font-weight:800;color:#F1F5F9;'>{_n}</div>"
                f"<div style='font-size:9px;color:#94A3B8;'>Communes</div>"
                f"</div>", unsafe_allow_html=True
            )

        # Carte Prioritaires — popover
        with kb:
            with st.popover(f"🟢 {_prio} Prio.", use_container_width=True):
                st.markdown("### 🟢 Zones Prioritaires")
                st.caption("Communes avec le meilleur score d'attractivité territoriale — opportunités d'investissement immédiates.")
                if not df_prio.empty:
                    for _, r in df_prio.iterrows():
                        sc = int(r["score_attractivite"]*100)
                        cho = r["taux_chomage"]
                        rev = int(r.get("revenu_median", 0))
                        bar_w = sc
                        st.markdown(
                            f"<div style='border:1px solid #D1FAE5;border-radius:8px;padding:8px 11px;margin-bottom:6px;background:#F0FDF4;'>"
                            f"<div style='display:flex;justify-content:space-between;align-items:center;'>"
                            f"<span style='font-weight:700;font-size:13px;color:#065F46;'>{r['ville']}</span>"
                            f"<span style='background:#16A34A;color:#fff;border-radius:20px;padding:2px 8px;font-size:11px;font-weight:700;'>{sc}%</span>"
                            f"</div>"
                            f"<div style='font-size:10px;color:#6B7280;margin-top:3px;'>{r.get('dept_nom','')}</div>"
                            f"<div style='background:#D1FAE5;border-radius:3px;height:4px;margin:5px 0 4px;'>"
                            f"<div style='width:{bar_w}%;height:4px;background:#16A34A;border-radius:3px;'></div></div>"
                            f"<div style='display:flex;gap:10px;font-size:10px;color:#6B7280;'>"
                            f"<span>💼 Chômage : <b>{cho:.1f}%</b></span>"
                            f"<span>💶 Revenu : <b>{rev:,} €</b></span>"
                            f"</div></div>",
                            unsafe_allow_html=True
                        )
                    st.info(f"💡 **{_prio} communes prioritaires** — Score attractivité ≥ 70%. Idéales pour commerce, santé, formation.")
                else:
                    st.warning("Aucune zone prioritaire détectée.")

        # Carte Signaux forts — popover
        with kc:
            with st.popover(f"⚡ {_sig} Sig.", use_container_width=True):
                st.markdown("### ⚡ Signaux Forts")
                st.caption("Territoires détectés par l'IA comme fragiles ou en mutation rapide — nécessitent une attention immédiate.")
                st.markdown(
                    "<div style='background:#FFFBEB;border:1px solid #FDE68A;border-radius:8px;"
                    "padding:8px 11px;margin-bottom:10px;font-size:11px;color:#92400E;'>"
                    "⚠️ <b>Qu'est-ce qu'un signal fort ?</b><br>"
                    "Commune avec chômage élevé, pauvreté croissante, perte de commerces ou "
                    "désertification médicale simultanée. Le modèle ML détecte ces situations "
                ...
                if not df_sig.empty:
                    for _, r in df_sig.iterrows():
                        sc   = int(r["score_signal_faible"]*100)
                        cho  = r.get("taux_chomage", 0)
                        pauv = r.get("taux_pauvrete", 0)
                        sev_color = "#DC2626" if sc >= 70 else "#D97706"
                        sev_bg    = "#FEE2E2" if sc >= 70 else "#FEF3C7"
                        sev_lbl   = "Critique" if sc >= 70 else "Élevé"
                        st.markdown(
                            f"<div style='border:1px solid {sev_color}44;border-radius:8px;padding:8px 11px;"
                            f"margin-bottom:6px;background:{sev_bg};'>"
                            f"<div style='display:flex;justify-content:space-between;align-items:center;'>"
                            f"<span style='font-weight:700;font-size:13px;color:#1E293B;'>{r['ville']}</span>"
                            f"<span style='background:{sev_color};color:#fff;border-radius:20px;"
                            f"padding:2px 8px;font-size:10px;font-weight:700;'>{sev_lbl} {sc}%</span>"
                            f"</div>"
                            f"<div style='font-size:10px;color:#6B7280;margin-top:3px;'>{r.get('dept_nom','')}</div>"
                            f"<div style='display:flex;gap:10px;font-size:10px;color:#6B7280;margin-top:4px;'>"
                            f"<span>💼 Chômage : <b style='color:{sev_color};'>{cho:.1f}%</b></span>"
                            f"<span>📉 Pauvreté : <b style='color:{sev_color};'>{pauv:.1f}%</b></span>"
                            f"</div></div>",
                            unsafe_allow_html=True
                        )
                    st.warning(f"🔴 **{_sig} signaux forts** — Ces communes nécessitent un plan d'action prioritaire.")
                else:
                    st.success("Aucun signal fort critique détecté.")

        # Carte Déserts médicaux — popover
        with kd:
            with st.popover(f"🏥 {_desert} Dés.", use_container_width=True):
                st.markdown("### 🏥 Déserts Médicaux")
                st.caption("Communes avec accès aux soins insuffisant — moins de 2,5 médecins généralistes pour 10 000 habitants.")
                st.markdown(
                    "<div style='background:#FFF1F2;border:1px solid #FECDD3;border-radius:8px;"
                    "padding:8px 11px;margin-bottom:10px;font-size:11px;color:#9F1239;'>"
                    "🩺 <b>Critère désert médical :</b><br>"
                    "Score désert ≥ 0.65 ou densité médicale &lt; 2,5 généralistes / 10k hab. "
                    "Ces zones représentent une opportunité directe pour l'installation de professionnels de santé."
                    "</div>",
                    unsafe_allow_html=True
                )
                if not df_des.empty:
                    for _, r in df_des.iterrows():
                        sc    = int(r["score_desert_medical"]*100)
                        med   = r.get("medecins_10k_hab", 0)
                        pop   = int(r.get("population", 0))
                        urgence = "🔴 Critique" if sc >= 80 else ("🟠 Sévère" if sc >= 65 else "🟡 Modéré")
                        st.markdown(
                            f"<div style='border:1px solid #FECDD3;border-radius:8px;padding:8px 11px;"
                            f"margin-bottom:6px;background:#FFF1F2;'>"
                            f"<div style='display:flex;justify-content:space-between;align-items:center;'>"
                            f"<span style='font-weight:700;font-size:13px;color:#1E293B;'>{r['ville']}</span>"
                            f"<span style='font-size:10px;font-weight:700;color:#DC2626;'>{urgence}</span>"
                            f"</div>"
                            f"<div style='font-size:10px;color:#6B7280;margin-top:3px;'>{r.get('dept_nom','')}</div>"
                            f"<div style='display:flex;gap:10px;font-size:10px;color:#6B7280;margin-top:4px;'>"
                            f"<span>🩺 <b style='color:#DC2626;'>{med:.1f}</b> méd./10k hab.</span>"
                            f"<span>👥 Pop. <b>{pop:,}</b></span>"
                            f"</div></div>",
                            unsafe_allow_html=True
                        )
                    st.error(f"🏥 **{_desert} déserts médicaux** — Opportunité d'installation pour médecins généralistes et spécialistes.")
                else:
                    st.success("Aucun désert médical critique détecté.")
