...
def page_deserts(df: pd.DataFrame):
    ...
    with fc5: aff_san = st.checkbox("🟠 Santé+",     value=False,
                help="Dentistes · Pharmacies · Infirmiers · Kinés")
    with fc6: niv     = st.selectbox("Niveau", ["Fort","Modéré","Tous"], index=0)
    ...
    pop_m = df_m["population"].clip(lower=1)

    # Scores Santé+ calculés ici pour la carte
    df_m["score_desert_dentiste"]   = (1 - ((df_m["nb_dentiste"]       * 10000 / pop_m) / 8 ).clip(0,1))
    df_m["score_desert_pharmacie"]  = (1 - ((df_m["nb_pharmacie"]      * 10000 / pop_m) / 5 ).clip(0,1))
    df_m["score_desert_infirmier"]  = (1 - ((df_m["nb_infirmier"]      * 10000 / pop_m) / 10).clip(0,1))
    df_m["score_desert_kine"]       = (1 - ((df_m["nb_kinesitherapeute"]* 10000 / pop_m) / 5 ).clip(0,1))
    df_m["score_desert_poste"]      = (1 - ((df_m["nb_poste"]          * 10000 / pop_m) / 3 ).clip(0,1))
    for sc in ["score_desert_dentiste","score_desert_pharmacie","score_desert_infirmier",
               "score_desert_kine","score_desert_poste"]:
        df_m[sc.replace("score_","cat_")] = df_m[sc].apply(
            lambda s: "Fort" if s>=.65 else ("Modéré" if s>=.40 else "Faible"))

    # Score Santé+ = pire des 5 sous-scores (plus pénalisant)
    df_m["score_desert_sante_plus"] = df_m[[
        "score_desert_dentiste","score_desert_pharmacie",
        "score_desert_infirmier","score_desert_kine","score_desert_poste"
    ]].max(axis=1)
    df_m["cat_desert_sante_plus"] = df_m["score_desert_sante_plus"].apply(
        lambda s: "Fort" if s>=.65 else ("Modéré" if s>=.40 else "Faible"))
    ...
    if not _FOLIUM_OK:
        ...
        for _, row in df_m.iterrows():
            ...
            if aff_san and row.get("cat_desert_sante_plus","") in niveaux_f:
                deserts.append(("santé+",     float(row.get("score_desert_sante_plus",0)), row.get("cat_desert_sante_plus","")))
            if not deserts:
                continue
            ...
            # Tooltip enrichi avec détail Santé+
            def _niv_badge(cat):
                c = "#DC2626" if cat=="Fort" else ("#D97706" if cat=="Modéré" else "#64748B")
                return f"<span style='color:{c};font-weight:700;'>{cat}</span>"

            lines = "".join(
                f"<div style='margin:2px 0;'>{DESERT_STYLE[d][1]} {_niv_badge(cat)}</div>"
                for d, _, cat in deserts
            )

            # Détail santé+ si activé
            san_detail = ""
            if aff_san:
                items = [
                    ("🦷 Dentistes",  int(row.get("nb_dentiste",0)),        row.get("cat_desert_dentiste","")),
                    ("💊 Pharmacies", int(row.get("nb_pharmacie",0)),       row.get("cat_desert_pharmacie","")),
                    ("💉 Infirmiers", int(row.get("nb_infirmier",0)),       row.get("cat_desert_infirmier","")),
                    ("🤸 Kinés",      int(row.get("nb_kinesitherapeute",0)),row.get("cat_desert_kine","")),
                    ("📮 Postes",     int(row.get("nb_poste",0)),           row.get("cat_desert_poste","")),
                ]
                san_detail = "<div style='margin-top:6px;padding-top:5px;border-top:1px solid #E2E8F2;font-size:10px;'>"
                for lbl, nb, cat in items:
                    san_detail += f"<div>{lbl} : <b>{nb}</b> — {_niv_badge(cat)}</div>"
                san_detail += "</div>"

            tooltip_html = (
                f"<div style='font-family:Plus Jakarta Sans,sans-serif;min-width:180px;'>"
                f"<b style='font-size:13px;'>{row.get('ville','')}</b>"
                f"<span style='color:#64748B;font-size:11px;margin-left:6px;'>{row.get('dept_nom','')}</span>"
                f"<div style='margin:5px 0 3px;'>{lines}</div>"
                f"<div style='font-size:10px;color:#94A3B8;'>👥 {safe_val(row.get('population',0))} hab. "
                f"· 👨‍⚕️ {int(row.get('nb_medecin_generaliste',0))} méd. gén."
                f"· 🏪 {int(row.get('nb_supermarche',0)+row.get('nb_epicerie',0))} commerces</div>"
                f"{san_detail}"
                f"</div>"
            )
            folium.CircleMarker(
    ...
    # ── TABLEAUX ─────────────────────────────────────────────────
    st.markdown("---")
    tabs_d = st.tabs(["🔴 Médical","🔵 Commercial","🟣 Mobilité","🎓 Scolaire","🟠 Santé+"])

    for tab, col, cat_col, extra_cols, extra_names in [
        (tabs_d[0], "score_desert_medical",    "cat_desert_medical",
         ["nb_medecin_generaliste","medecins_10k_hab"],
         ["Méd. gén.","Méd./10k"]),
        (tabs_d[1], "score_desert_commercial", "cat_desert_commercial",
         ["nb_supermarche","nb_epicerie","nb_boulangerie"],
         ["Supermarchés","Épiceries","Boulangeries"]),
        (tabs_d[2], "score_desert_mobilite",   "cat_desert_mobilite",
         ["nb_gares","gares_10k_hab"],
         ["Nb gares","Gares/10k"]),
    ]:
        with tab:
            cols_sel = ["ville","dept_nom",col,cat_col,"population"] + extra_cols
            t = df.nlargest(15, col)[cols_sel].copy()
            t.columns = ["Commune","Dép.","Score","Niveau","Population"] + extra_names
    ...
    with tabs_d[3]:
        if not has_bpe_sco:
            ...
            t = df.nlargest(15,"score_desert_primaire")[
                ["ville","dept_nom","nb_ecole_primaire","nb_college","nb_lycee",
                 "score_desert_primaire","cat_desert_primaire","population"]
            ].copy()
            t.columns = ["Commune","Dép.","Écoles primaires","Collèges","Lycées","Score","Niveau","Population"]
            st.dataframe(t.reset_index(drop=True), use_container_width=True)

    with tabs_d[4]:
        pop_t = df["population"].clip(lower=1)
        df_san = df.copy()
        df_san["score_dent"]  = (1 - ((df_san["nb_dentiste"]        * 10000 / pop_t) / 8 ).clip(0,1))
        df_san["score_phar"]  = (1 - ((df_san["nb_pharmacie"]       * 10000 / pop_t) / 5 ).clip(0,1))
        df_san["score_infi"]  = (1 - ((df_san["nb_infirmier"]       * 10000 / pop_t) / 10).clip(0,1))
        df_san["score_kine"]  = (1 - ((df_san["nb_kinesitherapeute"]* 10000 / pop_t) / 5 ).clip(0,1))
        df_san["score_poste"] = (1 - ((df_san["nb_poste"]           * 10000 / pop_t) / 3 ).clip(0,1))
        df_san["score_san_max"] = df_san[["score_dent","score_phar","score_infi","score_kine","score_poste"]].max(axis=1)

        def _cat(s): return "Fort" if s>=.65 else ("Modéré" if s>=.40 else "Faible")
        t_san = df_san.nlargest(15,"score_san_max")[
            ["ville","dept_nom","nb_dentiste","nb_pharmacie","nb_infirmier","nb_kinesitherapeute","nb_poste","population"]
        ].copy()
        t_san.columns = ["Commune","Dép.","Dentistes","Pharmacies","Infirmiers","Kinés","Postes","Population"]
        st.dataframe(t_san.reset_index(drop=True), use_container_width=True)
