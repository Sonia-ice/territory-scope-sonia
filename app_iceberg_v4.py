...
def page_attractivite(df: pd.DataFrame):
    ...
    with col_map:
        ...
        ZONE_COLOR_HEX = {
            ...
            "Zone Favorable":   "#3B82F6",
            "Zone Possible":    "#FBBF24",
            "Non Recommandé":   "#94A3B8",
        }

        lat_center = df_m["latitude"].mean()
        lon_center = df_m["longitude"].mean()
        ...
        if _FOLIUM_OK:
            ...
            if mode_carte == "Heatmap":
                ...
                heat_data = [
                    [row["latitude"], row["longitude"], row["score_custom"]]
                ...
                HeatMap(
                    ...
                    gradient={0.2: "#EFF6FF", 0.5: "#93C5FD", 0.75: "#3B82F6", 1.0: "#1A56DB"},
                ).add_to(m)
                # Top 15 labels
                for _, row in df_m.nlargest(15, "score_custom").iterrows():
                    folium.CircleMarker(
                        location=[row["latitude"], row["longitude"]],
                        radius=5, color="#1A56DB", fill=True,
                        fill_color="#1A56DB", fill_opacity=0.9, weight=1,
                        popup=folium.Popup(
                            f"<div style='font-family:sans-serif;min-width:160px;'>"
                            f"<b style='font-size:13px;'>{row['ville']}</b><br>"
                            f"<span style='background:#EBF1FF;color:#1344B8;padding:1px 7px;"
                            f"border-radius:20px;font-size:11px;font-weight:700;'>{row['cat_custom']}</span>"
                            f"<span style='font-size:13px;font-weight:700;color:#0A0F1E;margin-left:6px;'>{row['score_pct']}%</span><hr style='margin:5px 0;'/>"
                            f"<span style='font-size:11px;color:#555;'>Pop. {row['pop_fmt']} · Chôm. {row['cho_fmt']}% · Rev. {row['rev_fmt']} €</span>"
                            f"</div>",
                            max_width=240
                        ),
                        tooltip=row["ville"],
            ...
            else:  # Scatter ou 3D Colonnes → cercles colorés par zone
                for _, row in df_m.iterrows():
                    hex_col = ZONE_COLOR_HEX.get(row["cat_custom"], "#94A3B8")
                    radius_m = int(row["score_custom"] * 600 + 150) if mode_carte == "Scatter" else 280
                    folium.CircleMarker(
                        location=[row["latitude"], row["longitude"]],
                        radius=7,
                        color=hex_col,
                        ...
                        popup=folium.Popup(
                            ...
                            f"<span style='background:#EBF1FF;color:#1344B8;padding:1px 7px;"
                            f"border-radius:20px;font-size:11px;font-weight:700;'>{row['cat_custom']}</span>"
                            f"<span style='font-size:13px;font-weight:700;color:#0A0F1E;margin-left:6px;'>{row['score_pct']}%</span>"
                            f"<hr style='margin:5px 0;'/>"
                            f"<table style='font-size:11px;width:100%;'>"
                            f"<tr><td style='color:#888;'>Population</td><td style='font-weight:600;'>{row['pop_fmt']}</td></tr>"
                            f"<tr><td style='color:#888;'>Chômage</td><td style='font-weight:600;'>{row['cho_fmt']}%</td></tr>"
                            f"<tr><td style='color:#888;'>Revenu méd.</td><td style='font-weight:600;'>{row['rev_fmt']} €</td></tr>"
                            f"</table></div>",
                            max_width=240
                        ),
                        tooltip=f"{row['ville']} — {row['score_pct']}%",
                    ).add_to(m)

            st_folium(m, width=None, height=520, use_container_width=True, returned_objects=[])

        else:
            st.warning("⚠️ Folium non installé. Lancez : `pip install folium streamlit-folium`")
