...
def page_indicateurs(df: pd.DataFrame):
    ...
    with tab_vue:
        ...
        # ── Solde net mensuel (barres) ────────────────────────────
        bar_solde = alt.Chart(df_solde).mark_bar(
            cornerRadiusTopLeft=4, cornerRadiusTopRight=4
        ).encode(
            x=alt.X("Mois:O", sort=mois_labels,
                axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8",
                              domainColor="#E2E8F2", labelFontSize=10, title="")),
            y=alt.Y("Solde:Q",
                axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8",
                              domainColor="transparent", labelFontSize=10, title="")),
            color=alt.Color("Couleur:N",
                scale=alt.Scale(domain=["Positif","Négatif"], range=["#059669","#DC2626"]),
                legend=None),
            tooltip=["Mois:O","Solde:Q"]
        ).properties(height=160,
            title=alt.TitleParams("Solde net mensuel (Créations − Fermetures)",
            fontSize=13, fontWeight=700, color="#0A0F1E")
        ).configure_view(strokeWidth=0).configure_axis(labelFont="Plus Jakarta Sans")
