...
def _build_folium_map(df_m: pd.DataFrame, mode_carte: str, show_zones: list,
    ...
    if mode_carte == "Opportunités 2027":
        # Carte des meilleures opportunités projetées à 2027
        heat_data = [
        ...
        top20 = df_vis.nlargest(20, "opportunity_score" if "opportunity_score" in df_vis.columns else "score_custom")
        for _, row in top20.iterrows():
            cat = row["cat_custom"]
            hex_col = ZONE_HEX.get(cat, "#94A3B8")
            opp_pct = int(float(row.get("opportunity_score", row["score_custom"]))*100)
            ville_court = str(row['ville'])[:14] + ("…" if len(str(row['ville'])) > 14 else "")
            icon_html = (
                f"<div style='display:flex;flex-direction:column;align-items:center;gap:2px;'>"
                f"<div style='width:16px;height:16px;border-radius:50%;background:{hex_col};"
                f"border:2px solid #fff;cursor:pointer;display:flex;align-items:center;"
                f"justify-content:center;font-size:7px;font-weight:800;color:#fff;'>{opp_pct}</div>"
            ...
            folium.Marker(
                ...
                icon=folium.DivIcon(html=icon_html, icon_size=(80, 32), icon_anchor=(8, 8)),
...
def page_deserts(df: pd.DataFrame):
    ...
    if _FOLIUM_OK:
        ...
        lat_c = df_m["latitude"].mean(); lon_c = df_m["longitude"].mean()
        fm = _folium.Map(location=[lat_c, lon_c], zoom_start=10,
                         tiles="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",
                         attr="Esri", prefer_canvas=True)

        # CSS labels cachés jusqu'au zoom 13
        fm.get_root().html.add_child(_be.Element("""
        <style>
        .desert-label { display:none !important; }
        @keyframes pulse-red{0%,100%{box-shadow:0 0 0 0 rgba(220,38,38,.5)}50%{box-shadow:0 0 0 8px rgba(220,38,38,0)}}
        </style>
        <script>
        document.addEventListener('DOMContentLoaded',function(){
          var iv=setInterval(function(){
            if(window.L){clearInterval(iv);
              document.querySelectorAll('.leaflet-container').forEach(function(c){
                if(c._leaflet_map){c._leaflet_map.on('zoomend',function(){
                  var z=c._leaflet_map.getZoom();
                  document.querySelectorAll('.desert-label').forEach(function(el){
                    el.style.display=z>=13?'block':'none';
                  });
                });}
              });
        ...
        COLOR_MAP = {
            "med": ("#DC2626","rgba(220,38,38,"),
            "com": ("#1D4ED8","rgba(29,78,216,"),
            "mob": ("#7C3AED","rgba(124,58,237,"),
            "sco": ("#059669","rgba(22,163,74,"),
        }
        HEAT_GRADIENTS = {
            "med": {0.15:"#FEF2F2",0.40:"#FCA5A5",0.70:"#EF4444",1.0:"#991B1B"},
            "com": {0.15:"#EFF6FF",0.40:"#93C5FD",0.70:"#3B82F6",1.0:"#1D4ED8"},
            "mob": {0.15:"#F5F3FF",0.40:"#C4B5FD",0.70:"#8B5CF6",1.0:"#6D28D9"},
            "sco": {0.15:"#F0FDF4",0.40:"#86EFAC",0.70:"#22C55E",1.0:"#15803D"},
        }

        layers_info = []
        if aff_med:
            layers_info.append(("med", df_m[df_m["cat_desert_medical"].isin(niveaux_f)].copy(),
                                 "score_desert_medical","Désert médical","cat_desert_medical"))
        if aff_com:
            layers_info.append(("com", df_m[df_m["cat_desert_commercial"].isin(niveaux_f)].copy(),
                                 "score_desert_commercial","Désert commercial","cat_desert_commercial"))
        if aff_mob:
            layers_info.append(("mob", df_m[df_m["cat_desert_mobilite"].isin(niveaux_f)].copy(),
                                 "score_desert_mobilite","Désert mobilité","cat_desert_mobilite"))
        if aff_sco and has_bpe_sco:
            layers_info.append(("sco", df_m[df_m["cat_desert_primaire"].isin(niveaux_f)].copy(),
                                 "score_desert_primaire","Désert scolaire","cat_desert_primaire"))

        for key, dlay, score_col, label, cat_col in layers_info:
            if dlay.empty: continue
            hex_col, rgb_prefix = COLOR_MAP[key]
            # Heatmap
            heat_data = [[r["latitude"],r["longitude"],float(r[score_col])]
                         for _,r in dlay.iterrows()]
            _HeatMap(heat_data, radius=28, blur=20, max_zoom=13,
                     gradient=HEAT_GRADIENTS[key]).add_to(fm)
            # Marqueurs avec nom
            for _, row in dlay.iterrows():
                sc   = float(row[score_col])
                cat  = str(row[cat_col])
                ville_c = str(row["ville"])[:15]+("…" if len(str(row["ville"]))>15 else "")
                sev_color = hex_col if cat=="Fort" else ("#F59E0B" if cat=="Modéré" else "#94A3B8")
                icon_html = (
                    f"<div style='display:flex;flex-direction:column;align-items:center;gap:2px;'>"
                    f"<div style='width:10px;height:10px;border-radius:50%;"
                    f"background:{sev_color};border:1.5px solid #fff;opacity:0.9;'></div>"
                    f"<div class='desert-label' style='background:rgba(10,15,30,.80);color:#fff;"
                    f"padding:1px 4px;border-radius:3px;font-size:8px;font-weight:700;"
                    f"white-space:nowrap;font-family:sans-serif;'>{ville_c}</div>"
                    f"</div>"
                )
                _folium.Marker(
                    location=[row["latitude"], row["longitude"]],
                    icon=_folium.DivIcon(html=icon_html, icon_size=(90,22), icon_anchor=(5,5)),
                    popup=_folium.Popup(
                        f"<div style='font-family:sans-serif;min-width:170px;'>"
                        f"<b style='font-size:13px;'>{row['ville']}</b><br>"
                        f"<span style='background:#FEE2E2;color:{hex_col};padding:1px 7px;"
                        f"border-radius:20px;font-size:10px;font-weight:700;'>{label} · {cat}</span>"
                        f"<hr style='margin:6px 0;'/>"
                        f"<div style='font-size:11px;color:#555;'>"
                        f"Score : <b>{sc:.0%}</b><br>"
                        f"Pop. : <b>{safe_val(row.get('population',0))}</b></div></div>",
                        max_width=200),
                    tooltip=_folium.Tooltip(f"<b>{row['ville']}</b> · {label} {cat}",
                                            style="font-family:sans-serif;font-size:12px;"),
                ).add_to(fm)

        # Légende flottante
        legend_items = []
        if aff_med: legend_items.append(("#DC2626","Désert médical"))
        if aff_com: legend_items.append(("#1D4ED8","Désert commercial"))
        if aff_mob: legend_items.append(("#7C3AED","Désert mobilité"))
        if aff_sco: legend_items.append(("#059669","Désert scolaire"))
        leg_rows = "".join(
            f"<div style='display:flex;align-items:center;gap:6px;margin:3px 0;'>"
            f"<div style='width:10px;height:10px;border-radius:50%;background:{c};'></div>"
            f"<span style='font-size:10px;color:#1E293B;'>{l}</span></div>"
            for c,l in legend_items)
        fm.get_root().html.add_child(_be.Element(
            f"<div style='position:fixed;bottom:28px;left:12px;z-index:9999;"
            f"background:rgba(255,255,255,.94);border-radius:10px;"
            f"padding:10px 13px;box-shadow:0 4px 16px rgba(10,15,30,.18);"
            f"border:1px solid #E2E8F2;min-width:160px;'>"
            f"<div style='font-size:9px;font-weight:800;color:#8895AA;text-transform:uppercase;"
            f"letter-spacing:.8px;margin-bottom:6px;'>Couches actives</div>"
            f"{leg_rows}"
            f"<div style='font-size:8px;color:#CBD5E1;margin-top:6px;'>Noms visibles au zoom ≥ 13</div>"
            f"</div>"
        ))

        if layers_info:
            st_folium(fm, width=None, height=520, use_container_width=True, returned_objects=[])
        else:
            st.info("Sélectionnez au moins un type de désert.")
    else:
        # Fallback pydeck si folium absent
        layers = []
        if aff_med:
            ...
            layers.append(pdk.Layer("HeatmapLayer",data=d,get_position="[longitude,latitude]",
                get_weight="weight",radiusPixels=60,intensity=1.2,threshold=0.08,
                color_range=[[254,235,200,0],[253,141,60,120],[240,59,32,200],[189,0,38,255]]))
        view = pdk.ViewState(latitude=df_m["latitude"].mean(),longitude=df_m["longitude"].mean(),zoom=10,pitch=40)
        if layers:
            st.pydeck_chart(pdk.Deck(layers=layers,initial_view_state=view,
                tooltip={"html":"<b>{ville}</b>"},
...
def page_communes(df: pd.DataFrame):
    ...
    with tab_comp:
        ...
        try:
            ...
            df_2 = pd.DataFrame([
                {"latitude":lat_A,"longitude":lon_A,"ville":A,"color":[26,86,219,230],"dept":str(rA["dept_nom"])},
                {"latitude":lat_B,"longitude":lon_B,"ville":B,"color":[109,40,217,230],"dept":str(rB["dept_nom"])},
            ])
            ...
            st.info(f"📍 {A} · {rA.get('dept_nom','')} &nbsp;|&nbsp; 📍 {B} · {rB.get('dept_nom','')} — carte non disponible pour ces communes.")
