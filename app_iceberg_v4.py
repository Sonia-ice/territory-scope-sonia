...
def _build_popup_html(row: pd.Series) -> str:
    """Génère le HTML du popup riche avec jauges ML animées."""
    ...
    def bar(val, color, label):
        return (
            ...
            f"<span>{label}</span><span style='font-weight:700;color:{color};'>{val:.0f}%</span></div>"
            f"<div style='background:#E2E8F2;border-radius:4px;height:6px;overflow:hidden;'>"
            f"<div style='width:{val:.0f}%;height:6px;background:{color};border-radius:4px;"
            f"transition:width 0.8s ease;'></div></div></div>"
        )

    return (
        f"<div style='font-family:Plus Jakarta Sans,sans-serif;min-width:230px;max-width:260px;padding:2px;'>"
        # En-tête
        f"<div style='background:linear-gradient(135deg,#0B1F5C,#1A56DB);border-radius:10px 10px 0 0;"
        f"padding:10px 13px;margin:-4px -4px 10px;'>"
        ...
        f"</div></div>"
        # Cluster ML
        f"<div style='background:#EDE9FE;border-radius:6px;padding:5px 9px;margin-bottom:8px;"
        ...
        + f"</div>"
        # Données territoire
        f"<div style='border-top:1px solid #E2E8F2;padding-top:7px;"
...
def _build_folium_map(df_m: pd.DataFrame, mode_carte: str, show_zones: list) -> "folium.Map":
    ...
    if mode_carte == "Heatmap IA":
        ...
        for _, row in top20.iterrows():
            ...
            folium.Marker(
                ...
                popup=folium.Popup(_build_popup_html(row), max_width=270, show=False),
...
def page_attractivite(df: pd.DataFrame):
    ...
    with col_map:
        ...
        # ── Carte Folium ──────────────────────────────────────────
        if _FOLIUM_OK:
            folium_map = _build_folium_map(df_m, mode_carte, show_zones)
            st_folium(folium_map, width=None, height=560,
                      use_container_width=True, returned_objects=[])
        else:
            st.error("⚠️ Folium non installé. Lancez : `pip install folium streamlit-folium`")
    ...
    with col_panel:
        ...
        # Signaux faibles
        top_sig  = df.nlargest(8,"score_signal_faible")[
    ...
    with c2:
        all_s = df[["ville","dept_nom","score_custom","cat_custom","opportunity_score","ml_cluster"]
