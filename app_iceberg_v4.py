...
<style>
/* ════════════════════════════════════════════════════════════
   ICEBERG v5 — PREMIUM DARK DESIGN SYSTEM
   Inspired by: Linear · Vercel · OpenAI · Stripe
   Font: Syne (display) + DM Sans (body) + JetBrains Mono
   Theme: Deep space dark · Electric cyan accent · Glassmorphism
════════════════════════════════════════════════════════════ */
...
:root {
  ...
  /* Shadows */
  --sh:     0 1px 3px rgba(0,0,0,.40), 0 2px 8px rgba(0,0,0,.30);
  --sh-md:  0 4px 20px rgba(0,0,0,.50), 0 1px 4px rgba(0,0,0,.30);
  --sh-lg:  0 8px 40px rgba(0,0,0,.60), 0 4px 12px rgba(0,0,0,.40);
  --sh-glow:0 0 0 2px var(--accent-glow), 0 0 20px rgba(0,212,255,.10);
  --sh-accent: 0 4px 24px rgba(0,212,255,.25), 0 0 0 1px rgba(0,212,255,.15);
}
...
.stApp::before {
  ...
  background:
    radial-gradient(ellipse 80% 50% at 20% 0%,  rgba(0,212,255,.06) 0%, transparent 60%),
    radial-gradient(ellipse 60% 40% at 80% 100%, rgba(139,92,246,.05) 0%, transparent 60%),
    radial-gradient(ellipse 40% 30% at 50% 50%,  rgba(16,185,129,.03) 0%, transparent 70%);
}
...
[data-testid="stSidebar"] {
  background: linear-gradient(180deg, #060B18 0%, #08101F 40%, #060D1A 100%) !important;
  border-right: 1px solid rgba(0,212,255,.08) !important;
  transform: none !important; min-width: 292px !important; max-width: 308px !important;
}
...
[data-testid="stSidebar"] .stRadio label {
  ...
  gap: 9px !important; transition: all .18s ease !important;
...
h1 {
  ...
  color: var(--text-1) !important; letter-spacing: -.8px !important;
  line-height: 1.2 !important;
}
h2 {
  font-family: 'Syne', sans-serif !important;
  font-size: 20px !important; font-weight: 700 !important;
  color: var(--text-1) !important; letter-spacing: -.4px !important;
}
h3 {
...
[data-testid="metric-container"]::before {
  ...
  background: linear-gradient(135deg, var(--glass-2) 0%, transparent 100%);
  pointer-events:none;
}
...
[data-testid="metric-container"] label {
  ...
  letter-spacing: 1px !important; font-family: 'DM Sans', sans-serif !important;
}
[data-testid="metric-container"] [data-testid="stMetricValue"] {
  font-family: 'Syne', sans-serif !important;
  color: var(--text-1) !important; font-size: 28px !important;
...
.stButton > button {
  ...
  color: var(--text-inv) !important; border: none !important;
  border-radius: 10px !important;
  font-family: 'DM Sans', sans-serif !important;
  font-weight: 600 !important; font-size: 13px !important;
  padding: 10px 20px !important; letter-spacing: -.1px !important;
  box-shadow: 0 2px 12px rgba(0,212,255,.30) !important;
  transition: all .18s cubic-bezier(.4,0,.2,1) !important;
}
.stButton > button:hover {
  box-shadow: 0 4px 24px rgba(0,212,255,.50) !important;
...
.stMultiSelect > div > div {
  ...
  color: var(--text-1) !important; transition: all .15s ease !important;
}
...
.stTabs [data-baseweb="tab-list"] {
  background: var(--surface); border-radius: 12px; padding: 4px; gap: 2px;
...
.stTabs [data-baseweb="tab"] {
  ...
  transition: all .15s ease !important; letter-spacing: -.1px !important;
}
...
.stDataFrame tr:hover td { background: var(--glass) !important; }
...
[data-testid="stExpander"] summary {
  background: var(--surface) !important;
  color: var(--text-1) !important; font-weight: 500 !important;
...
::-webkit-scrollbar-thumb:hover { background: rgba(0,212,255,.30); }

/* ── MAPS & CHARTS ── */
.stDeckGlJsonChart {
  border-radius: var(--r-lg) !important; overflow: hidden !important;
  border: 1px solid var(--border) !important; box-shadow: var(--sh-lg) !important;
}
.stVegaLiteChart, .element-container iframe {
  border-radius: var(--r) !important;
...
/* ── ICEBERG COMPONENT LIBRARY ── */

/* Page header banner */
.ib-header {
  background: linear-gradient(135deg, #060F26 0%, #0A1A3A 50%, #071630 100%);
  border: 1px solid rgba(0,212,255,.12);
  border-radius: var(--r-xl); padding: 28px 32px; margin-bottom: 28px;
  position: relative; overflow: hidden;
  box-shadow: 0 8px 40px rgba(0,0,0,.50), 0 0 0 1px rgba(0,212,255,.06);
}
.ib-header::before {
...
.iceberg-card {
  ...
  box-shadow: var(--sh); transition: all .22s cubic-bezier(.4,0,.2,1);
  position: relative; overflow: hidden;
}
.iceberg-card::before {
...
.timeline-bar {
  ...
  background: var(--surface-3); margin: 4px 0;
}
.timeline-fill {
...
.ib-separator::before, .ib-separator::after {
...
.ib-count { animation: count-up .5s ease both; }
...
.stApp .main > div > div > div {
  animation: page-in .35s cubic-bezier(.4,0,.2,1) both;
}

/* Sidebar active nav indicator */
@keyframes nav-glow {
  0%, 100% { box-shadow: 0 0 8px rgba(0,212,255,.20); }
  50% { box-shadow: 0 0 16px rgba(0,212,255,.40); }
}
...
def page_header(icon: str, title: str, subtitle: str, badge: str = ""):
    ...
    st.markdown(
        f'<div class="ib-header">'
        f'<div style="display:flex;align-items:center;gap:16px;position:relative;z-index:1;">'
        f'<div style="width:52px;height:52px;background:linear-gradient(135deg,rgba(0,212,255,.20),rgba(0,212,255,.08));'
        f'border:1px solid rgba(0,212,255,.25);border-radius:16px;'
        f'display:flex;align-items:center;justify-content:center;font-size:24px;flex-shrink:0;'
        f'box-shadow:0 0 20px rgba(0,212,255,.15);">{icon}</div>'
        f'<div style="flex:1;">'
        f'<div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">'
        f'<span style="font-family:Syne,sans-serif;font-size:22px;font-weight:800;color:#F0F6FF;letter-spacing:-.5px;">{title}</span>'
        f'{badge_html}</div>'
        f'<div style="font-size:12px;color:rgba(136,153,180,.70);letter-spacing:.1px;">{subtitle}</div>'
        f'</div>'
        f'<div style="display:flex;align-items:center;gap:6px;opacity:.6;">'
        f'<span class="status-dot"></span>'
        f'<span style="font-size:10px;color:var(--green);font-weight:600;letter-spacing:.3px;">LIVE</span>'
        f'</div>'
        f'</div></div>',
        unsafe_allow_html=True
    )

def metric_card(label: str, value: str, delta: str = "", color: str = "var(--accent)",
                bg: str = "var(--surface)") -> str:
    delta_html = ""
    if delta:
        is_up = delta.startswith("↗") or delta.startswith("+")
        d_color = "var(--red)" if is_up else "var(--green)"
        delta_html = (f'<div style="font-size:11px;color:{d_color};font-weight:500;'
                      f'margin-top:6px;display:flex;align-items:center;gap:4px;">{delta}</div>')
    return (
        f'<div class="iceberg-card">'
        f'<div style="font-size:9px;font-weight:600;color:var(--text-3);'
        ...
        f'color:{color};letter-spacing:-.8px;line-height:1;">{value}</div>'
        f'{delta_html}</div>'
    )
...
def page_login(df: pd.DataFrame):
    ...
    @keyframes glow-pulse { 0%,100%{box-shadow:0 0 30px rgba(0,212,255,.15),0 0 0 1px rgba(0,212,255,.12)}
                            50%{box-shadow:0 0 60px rgba(0,212,255,.30),0 0 0 1px rgba(0,212,255,.25)} }
    @keyframes orbit {
    ...
    .hero-logo { animation: glow-pulse 3s ease-in-out infinite; }
    .hero-title { animation: fadeUp .6s cubic-bezier(.4,0,.2,1) .1s both; }
    .hero-sub   { animation: fadeUp .6s cubic-bezier(.4,0,.2,1) .2s both; }
    .hero-divider{animation: fadeIn .6s ease .3s both; }
    .hero-cards { animation: fadeUp .6s cubic-bezier(.4,0,.2,1) .35s both; }
    .hero-podium{ animation: fadeUp .6s cubic-bezier(.4,0,.2,1) .5s both; }
    .login-card {
      background: var(--surface);
      border: 1px solid var(--border);
    ...
    .login-card::before {
      content:''; position:absolute; inset:0;
      background: linear-gradient(135deg, var(--glass-2) 0%, transparent 60%);
      pointer-events:none;
    }
    .login-card:hover {
      border-color: var(--border-accent);
    ...
    <div style="display:flex;flex-direction:column;align-items:center;padding:48px 20px 20px;max-width:900px;margin:0 auto;">
      ...
      <div style="position:relative;width:140px;height:140px;display:flex;
        ...
        <div style="position:absolute;inset:0;border-radius:50%;
                    border:1px dashed rgba(0,212,255,.15);"></div>
        <div style="position:absolute;inset:16px;border-radius:50%;
                    border:1px dashed rgba(0,212,255,.10);"></div>
        <!-- Orbit dots -->
        <div style="position:absolute;width:8px;height:8px;border-radius:50%;
                    background:var(--accent);box-shadow:0 0 10px var(--accent);
                    animation:orbit 6s linear infinite;top:50%;left:50%;margin:-4px;"></div>
        <div style="position:absolute;width:5px;height:5px;border-radius:50%;
                    background:var(--purple);box-shadow:0 0 8px var(--purple);
                    animation:orbit2 9s linear infinite;top:50%;left:50%;margin:-2.5px;"></div>
        <div style="position:absolute;width:6px;height:6px;border-radius:50%;
                    background:var(--green);box-shadow:0 0 8px var(--green);
                    animation:orbit3 12s linear infinite;top:50%;left:50%;margin:-3px;"></div>
        <!-- Central logo -->
        <div class="hero-logo" style="width:76px;height:76px;
             background:linear-gradient(135deg,#001833,#003366,#001833);
             border:1px solid rgba(0,212,255,.30);
             border-radius:22px;display:flex;align-items:center;
             justify-content:center;font-size:34px;z-index:1;
             animation:glow-pulse 3s ease-in-out infinite, float 4s ease-in-out infinite;">🧊</div>
      </div>

      <!-- Title -->
      <div class="hero-title" style="text-align:center;margin-bottom:12px;">
        <div style="font-family:Syne,sans-serif;font-size:52px;font-weight:800;
                    color:#F0F6FF;letter-spacing:-2.5px;line-height:1;margin-bottom:4px;">
          ICEBERG
        </div>
        <div style="font-size:13px;color:var(--text-3);letter-spacing:3px;
                    text-transform:uppercase;font-weight:500;">
          Intelligence Territoriale · v5.0
        </div>
      </div>

      <!-- Accent line -->
      <div class="hero-divider" style="width:60px;height:2px;
           background:linear-gradient(90deg,transparent,var(--accent),transparent);
           border-radius:2px;margin:18px 0;opacity:.7;"></div>

      <!-- Stats chips -->
      <div class="hero-sub" style="display:flex;gap:10px;flex-wrap:wrap;
           justify-content:center;margin-bottom:8px;">
        <div class="stat-chip">🗺️ <b>241</b> communes</div>
        <div class="stat-chip">🤖 <b>ML</b> Gradient Boosting</div>
        <div class="stat-chip">🔮 Prédictions <b>2030</b></div>
        <div class="stat-chip">📍 Dép. <b>91</b> &amp; <b>94</b></div>
      </div>
      <div class="hero-divider" style="font-size:13px;color:var(--text-2);
           margin:16px 0 32px;font-weight:500;">Choisissez votre espace de travail</div>
    ...
    with mid:
        ca, cb = st.columns(2)
        for col, role_key, ico, titre, desc, features, btn_key, accent_color in [
            (ca, "directeur", "🏢", "Directeur d'Agence",
             "Accès complet aux analyses territoriales",
             ["⭐ Carte attractivité ML", "🔮 Prédictions 2030", "📋 Rapports PDF", "💡 Opportunités IA"],
             "btn_dir", "#00D4FF"),
            (cb, "administrateur", "⚙️", "Administrateur",
             "Gestion opérationnelle du territoire",
             ["⚡ Alertes temps réel", "🏥 Carte des déserts", "📊 Classement communes", "💬 Assistant IA"],
             "btn_adm", "#8B5CF6"),
        ]:
            with col:
                features_html = "".join(
                ...
                st.markdown(f"""
                <div class="login-card" style="border-top:2px solid {accent_color}33;">
                  <div style="width:60px;height:60px;margin:0 auto 18px;
                       background:linear-gradient(135deg,{accent_color}22,{accent_color}0a);
                       border:1px solid {accent_color}33;border-radius:18px;
                       display:flex;align-items:center;justify-content:center;font-size:28px;">
                    {ico}
                  </div>
                  <div style="font-family:Syne,sans-serif;font-size:17px;font-weight:700;
                              color:var(--text-1);margin-bottom:6px;">{titre}</div>
                  <div style="font-size:12px;color:var(--text-2);
                              line-height:1.5;margin-bottom:18px;">{desc}</div>
                  <div style="text-align:left;margin-bottom:20px;font-size:11px;color:var(--text-2);">
                    {features_html}
                  </div>
                </div>""", unsafe_allow_html=True)
                st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
                if st.button(f"Accéder →", key=btn_key, use_container_width=True):
    ...
    with mid2:
        ...
        <div style="text-align:center;margin-bottom:16px;">
          <div style="font-family:Syne,sans-serif;font-size:15px;font-weight:700;
               color:var(--text-1);letter-spacing:-.2px;">🏙️ Villes les plus actives</div>
          <div style="font-size:11px;color:var(--text-3);margin-top:4px;">
            Classement en temps réel · activité terrain</div>
        </div>""", unsafe_allow_html=True)
        ...
        if top3:
            ...
            medals = [("🥇","#F59E0B",True),("🥈","#8899B4",False),("🥉","#B45309",False)]
            ...
            for i, idx in enumerate(podium_order):
                ...
                med, clr, is_win = medals[idx]
                pct = int(data_p["score"]/max_score*100)
                with cols_p[i]:
                    crit_badge = (
                        f'<span style="background:var(--red-lt);color:var(--red);padding:1px 7px;'
                        f'border-radius:20px;font-size:9px;font-weight:600;">'
                        f'🔴 {data_p["critiques"]}</span>'
                    ) if data_p["critiques"] else ""
                    st.markdown(
                        f'<div style="background:var(--surface);border:1px solid var(--border);'
                        f'{"border-top:2px solid #F59E0B;" if is_win else ""}'
                        f'border-radius:16px;padding:20px 14px 16px;text-align:center;'
                        f'box-shadow:var(--sh);margin-top:{0 if is_win else 20}px;'
                        f'transition:all .2s ease;">'
                        f'<div style="font-size:{"34px" if is_win else "26px"};margin-bottom:8px;">{med}</div>'
                        f'<div style="font-family:Syne,sans-serif;font-size:{"15px" if is_win else "13px"};'
                        f'font-weight:700;color:var(--text-1);margin-bottom:6px;">{ville_p}</div>'
                        f'<div style="display:flex;justify-content:center;gap:5px;flex-wrap:wrap;margin:6px 0;">'
                        f'<span style="background:var(--accent-dim);color:var(--accent);padding:1px 7px;'
                        f'border-radius:20px;font-size:9px;font-weight:600;">{data_p["nb"]} alerte(s)</span>'
                        f'{crit_badge}</div>'
                        f'<div style="background:var(--surface-3);border-radius:4px;height:3px;margin-top:10px;">'
                        f'<div style="width:{pct}%;height:3px;background:linear-gradient(90deg,var(--accent-2),var(--accent));'
                        f'border-radius:4px;box-shadow:0 0 8px var(--accent-glow);"></div></div>'
                        f'<div style="font-size:10px;color:var(--text-3);margin-top:5px;font-weight:500;">'
                        f'{", ".join(list(data_p["admins"])[:2])}</div>'
                        f'</div>',
                        unsafe_allow_html=True
                    )
