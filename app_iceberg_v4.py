...
def page_attractivite(df: pd.DataFrame):
    ...
    if annee_sel > 2025:
        st.markdown(
            ...
            f"<span><b>Projection IA {annee_sel}</b> — Les zones sont recalculées par le modèle "
            f"Gradient Boosting selon les tendances ML de chaque profil de commune. "
            f"Les popups affichent la trajectoire complète 2025→2030.</span>"
            f"</div>",
            unsafe_allow_html=True
        )
