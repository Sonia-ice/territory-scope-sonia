...
[data-testid="stSidebar"] .stRadio label {
  ...
  color: #E2E8F0 !important; font-size: 13px !important; font-weight: 500 !important;
  display: flex !important; align-items: center !important; gap: 9px !important;
  transition: all .15s !important;
}
[data-testid="stSidebar"] .stRadio label:hover {
  background: rgba(255,255,255,.09) !important; color: #FFFFFF !important;
}
...
def page_deserts(df: pd.DataFrame):
    ...
    if not _FOLIUM_OK:
        ...
        # Couleurs par type de désert
        DESERT_COLORS = {
            "médical":    ("#DC2626", "#FEE2E2", "🔴"),
            "commercial": ("#1D4ED8", "#DBEAFE", "🔵"),
            "mobilité":   ("#7C3AED", "#EDE9FE", "🟣"),
            "scolaire":   ("#059669", "#D1FAE5", "🎓"),
        }

        def _get_desert_type(row):
            """Détermine le pire type de désert pour cette commune."""
            scores = {}
            if aff_med and row.get("cat_desert_medical","") in niveaux_f:
                scores["médical"] = float(row.get("score_desert_medical", 0))
            if aff_com and row.get("cat_desert_commercial","") in niveaux_f:
                scores["commercial"] = float(row.get("score_desert_commercial", 0))
            if aff_mob and row.get("cat_desert_mobilite","") in niveaux_f:
                scores["mobilité"] = float(row.get("score_desert_mobilite", 0))
            if aff_sco and has_bpe_sco and row.get("cat_desert_primaire","") in niveaux_f:
                scores["scolaire"] = float(row.get("score_desert_primaire", 0))
            if not scores:
                return None, None, None
            best = max(scores, key=scores.get)
            color_hex, color_bg, emoji = DESERT_COLORS[best]
            return best, color_hex, color_bg

        # Collecter les communes à afficher
        communes_shown = set()
        for _, row in df_m.iterrows():
            if pd.isna(row.get("latitude")) or pd.isna(row.get("longitude")):
                continue
            dtype, color_hex, color_bg = _get_desert_type(row)
            if dtype is None:
                continue
            ville_n = str(row.get("ville", ""))
            if ville_n in communes_shown:
                continue
            communes_shown.add(ville_n)

            # Niveau du désert
            niveau_labels = []
            if aff_med and row.get("cat_desert_medical","") in niveaux_f:
                niveau_labels.append(f"🔴 Médical : {row.get('cat_desert_medical','')}")
            if aff_com and row.get("cat_desert_commercial","") in niveaux_f:
                niveau_labels.append(f"🔵 Commercial : {row.get('cat_desert_commercial','')}")
            if aff_mob and row.get("cat_desert_mobilite","") in niveaux_f:
                niveau_labels.append(f"🟣 Mobilité : {row.get('cat_desert_mobilite','')}")
            if aff_sco and has_bpe_sco and row.get("cat_desert_primaire","") in niveaux_f:
                niveau_labels.append(f"🎓 Scolaire : {row.get('cat_desert_primaire','')}")

            # Marqueur pin avec nom visible
            pin_html = (
                f"<div style='display:flex;flex-direction:column;align-items:center;'>"
                f"<div style='background:{color_hex};color:#fff;padding:3px 8px;"
                f"border-radius:14px;font-size:10px;font-weight:700;"
                f"white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,.30);"
                f"font-family:Plus Jakarta Sans,sans-serif;border:1.5px solid rgba(255,255,255,.4);'>"
                f"{ville_n}</div>"
                f"<div style='width:1.5px;height:6px;background:{color_hex};'></div>"
                f"<div style='width:6px;height:6px;border-radius:50%;background:{color_hex};"
                f"border:1.5px solid #fff;'></div>"
                f"</div>"
            )
            popup_html = (
                f"<div style='font-family:sans-serif;min-width:160px;padding:2px;'>"
                f"<b style='font-size:13px;color:#0A0F1E;'>{ville_n}</b><br>"
                f"<span style='font-size:11px;color:#64748B;'>{row.get('dept_nom','')}</span><br>"
                f"<div style='margin-top:6px;'>{'<br>'.join(niveau_labels)}</div>"
                f"<div style='margin-top:6px;font-size:11px;color:#475569;'>"
                f"👥 {safe_val(row.get('population',0))} hab.</div>"
                f"</div>"
            )
            folium.Marker(
                ...
                icon=folium.DivIcon(
                    html=pin_html,
                    icon_size=(max(80, len(ville_n) * 8), 42),
                    icon_anchor=(max(40, len(ville_n) * 4), 42),
                ),
                popup=folium.Popup(popup_html, max_width=220),
                tooltip=folium.Tooltip(
                    f"<b>{ville_n}</b><br>" + "<br>".join(niveau_labels),
                    style="font-family:sans-serif;font-size:11px;"
                ),
            ).add_to(m_desert)

        st_folium(m_desert, width=None, height=520, use_container_width=True, returned_objects=[])
