...
def _get_contacts_mairie(ville: str, code_insee: str) -> dict:
    """Récupère les infos de la mairie via l'annuaire service-public.fr"""
    ...
    try:
        ...
        tel_list = [c.get("valeur","") for c in item.get("coordCartographie",[]) if c.get("type")=="Téléphone"] if item else []
        email_list = [c.get("valeur","") for c in item.get("coordCartographie",[]) if c.get("type")=="Email"] if item else []
        return {
            "nom": item.get("nom", f"Mairie de {ville}"),
            "adresse": item.get("adresseCourriel",""),
        ...
        for col, type_s, titre_s in [
            ...
            if type_s == "Emploi":
                ...
                contacts = [
                    ...
                    {"ini":"ML","nom":"Mission Locale","poste":f"Département {code_dep}",
                     "clr":"#6D28D9","tags":["Jeunes","16-25 ans"],
                     "note":"Accompagnement jeunes décrocheurs et NEET",
                     "url":"https://www.unml.info/le-reseau/annuaire/","tel":"—"},
                ]
            elif type_s == "Médical":
