...
def page_deserts(df: pd.DataFrame):
    ...
    # ── Bandeau source des données ────────────────────────────────
    pct_simule = int(df.get("donnees_bpe_simulees", pd.Series([False]*len(df))).sum() / len(df) * 100) if len(df) > 0 else 0
    if pct_simule > 50:
        st.info(
            f"ℹ️ **Données simulées** — {pct_simule}% des données BPE (commerces, médecins, écoles) sont "
            f"des estimations statistiques faute de données terrain disponibles en open data. "
            f"Les scores médicaux et commerciaux proviennent du modèle interne du CSV. "
            f"La mobilité est calculée depuis la densité de gares et la densité de population.",
            icon=None
        )
