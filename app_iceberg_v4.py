...
def page_indicateurs(df: pd.DataFrame):
    ...
    moy_rev   = float(df["revenu_median"].mean())        # €/mois
    ...
    taux_surv = float(df["taux_survie_entreprises"].mean()) * 100  # en %
    ent_1000  = float(df["entreprises_1000hab"].mean())
    dyn_eco   = float(df["dynamique_eco"].mean())        # score 0-100
    nb_trans  = int(df["nb_transactions"].sum())
    mois = ["Jan","Fév","Mar","Avr","Mai","Jun","Jul","Aoû","Sep","Oct","Nov","Déc"]

    # ── Répartition par tranche de chômage (données réelles) ──────
    tranches_cho = pd.cut(df["taux_chomage"],
        bins=[0,6,10,15,100], labels=["< 6%","6–10%","10–15%","> 15%"])
    dist_cho = tranches_cho.value_counts().sort_index()

    # ── Répartition dynamique éco (données réelles) ───────────────
    tranches_dyn = pd.cut(df["dynamique_eco"],
        bins=[0,50,65,80,101], labels=["Faible (<50)","Moyen (50-65)","Bon (65-80)","Fort (>80)"])
    dist_dyn = tranches_dyn.value_counts().sort_index()
    ...
    with tab_vue:
        ...
        with c2: st.markdown(metric_card("Entreprises actives", safe_val(nb_ent),
            f"Total créées : {safe_val(nb_ent_tot)}", "#059669"), unsafe_allow_html=True)
        with c3: st.markdown(metric_card("Taux de survie moy.", f"{taux_surv:.1f}%",
            f"Min {df['taux_survie_entreprises'].min()*100:.0f}% · Max {df['taux_survie_entreprises'].max()*100:.0f}%", "#1A56DB"), unsafe_allow_html=True)
        ...
        # ── Graphique répartition chômage par commune ─────────────
        df_cho_hist = df[["ville","taux_chomage","dept_nom"]].copy()
        hist_cho = alt.Chart(df_cho_hist).mark_bar(
            cornerRadiusTopLeft=3, cornerRadiusTopRight=3, color="#DC2626", opacity=0.75
        ).encode(
            x=alt.X("taux_chomage:Q", bin=alt.Bin(step=1),
                axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8",
                              domainColor="#E2E8F2", labelFontSize=10, title="Taux de chômage (%)")),
            y=alt.Y("count():Q",
                axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8",
                              domainColor="transparent", labelFontSize=10, title="Nb communes")),
            tooltip=["taux_chomage:Q", "count():Q"]
        ).properties(height=200,
            title=alt.TitleParams("Distribution du taux de chômage · 241 communes (données réelles)",
            fontSize=13, fontWeight=700, color="#0A0F1E")
        ).configure_view(strokeWidth=0).configure_axis(labelFont="Plus Jakarta Sans")
        st.altair_chart(hist_cho, use_container_width=True)

        # ── Scatter chômage vs revenu médian ─────────────────────
        scatter = alt.Chart(df).mark_circle(size=60, opacity=0.7).encode(
            x=alt.X("taux_chomage:Q",
                axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8",
                              labelFontSize=10, title="Taux de chômage (%)")),
            y=alt.Y("revenu_median:Q",
                axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8",
                              labelFontSize=10, title="Revenu médian (€/mois)")),
            color=alt.Color("dept_nom:N",
                scale=alt.Scale(domain=["Essonne","Val-de-Marne"],
                                range=["#1A56DB","#DC2626"]),
                legend=alt.Legend(orient="top-right", labelFontSize=10)),
            tooltip=["ville:N","dept_nom:N","taux_chomage:Q","revenu_median:Q","population:Q"]
        ).properties(height=220,
            title=alt.TitleParams("Chômage vs Revenu médian par commune",
            fontSize=13, fontWeight=700, color="#0A0F1E")
        ).configure_view(strokeWidth=0).configure_axis(labelFont="Plus Jakarta Sans")
    ...
    with tab_emp:
        ...
        with e1: st.markdown(metric_card("Chômage moy.",
            f"{moy_cho:.1f}%",
            f"Médiane : {df['taux_chomage'].median():.1f}%", "#DC2626"), unsafe_allow_html=True)
        ...
        with e3: st.markdown(metric_card("Taux pauvreté moy.",
            f"{moy_pauv:.1f}%",
            f"Médiane : {df['taux_pauvrete'].median():.1f}%", "#DC2626"), unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)

        # ── Top 15 communes chômage élevé ─────────────────────────
        top_cho = df.nlargest(15,"taux_chomage")[
            ["ville","dept_nom","taux_chomage","revenu_median","taux_pauvrete","population"]
        ].copy()
        top_cho.columns = ["Commune","Département","Chômage %","Revenu médian €/mois","Pauvreté %","Population"]
        top_cho["Chômage %"] = top_cho["Chômage %"].round(1)
        top_cho["Pauvreté %"] = top_cho["Pauvreté %"].round(1)
        top_cho["Revenu médian €/mois"] = top_cho["Revenu médian €/mois"].round(0).astype(int)
        ...
        # ── Répartition pauvreté par tranche ─────────────────────
        st.markdown("<br>", unsafe_allow_html=True)
        df_pauv = df[["ville","taux_pauvrete","dept_nom"]].copy()
        bar_pauv = alt.Chart(df_pauv).mark_bar(
            cornerRadiusTopLeft=3, cornerRadiusTopRight=3, opacity=0.8
        ).encode(
            x=alt.X("taux_pauvrete:Q", bin=alt.Bin(step=2),
                axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8",
                              labelFontSize=10, title="Taux de pauvreté (%)")),
            y=alt.Y("count():Q",
                axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8",
                              labelFontSize=10, title="Nb communes")),
            color=alt.condition(
                alt.datum.taux_pauvrete >= 20,
                alt.value("#DC2626"), alt.value("#3B82F6")),
            tooltip=["taux_pauvrete:Q","count():Q"]
        ).properties(height=180,
            title=alt.TitleParams("Distribution du taux de pauvreté (rouge = seuil critique > 20%)",
            fontSize=13, fontWeight=700, color="#0A0F1E")
        ).configure_view(strokeWidth=0).configure_axis(labelFont="Plus Jakarta Sans")
        st.altair_chart(bar_pauv, use_container_width=True)
    ...
    with tab_eco:
        ...
        with ec2: st.markdown(metric_card("Taux de survie moy.", f"{taux_surv:.1f}%",
            f"{int((df['taux_survie_entreprises']==1.0).sum())} communes à 100%", "#059669"), unsafe_allow_html=True)
        ...
        # ── Scatter prix m² vs revenu médian ─────────────────────
        sc_eco = alt.Chart(df).mark_circle(size=55, opacity=0.7).encode(
            x=alt.X("revenu_median:Q",
                axis=alt.Axis(labelColor="#8895AA", labelFontSize=10,
                              gridColor="#F0F4F8", title="Revenu médian (€/mois)")),
            y=alt.Y("prix_m2_median:Q",
                axis=alt.Axis(labelColor="#8895AA", labelFontSize=10,
                              gridColor="#F0F4F8", title="Prix m² médian (€)")),
            color=alt.Color("dept_nom:N",
                scale=alt.Scale(domain=["Essonne","Val-de-Marne"],
                                range=["#1A56DB","#DC2626"]),
                legend=alt.Legend(orient="top-left", labelFontSize=10)),
            size=alt.Size("population:Q", legend=None),
            tooltip=["ville:N","dept_nom:N","revenu_median:Q","prix_m2_median:Q","population:Q"]
        ).properties(height=220,
            title=alt.TitleParams("Revenu médian vs Prix immobilier par commune (taille = population)",
            fontSize=13, fontWeight=700, color="#0A0F1E")
        ).configure_view(strokeWidth=0).configure_axis(labelFont="Plus Jakarta Sans")
