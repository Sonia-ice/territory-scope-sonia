...
def page_login(df: pd.DataFrame):
    ...
    with mid2:
        ...
        # Utilise le classement cumulatif persistant (indépendant des alertes en mémoire)
        villes_scores = st.session_state.scores_villes_cumul
...
def page_classement(df: pd.DataFrame):
    ...
    with tab_rank:
        ...
        with sub_villes:
            # Classement cumulatif persistant — ne se réinitialise pas
            villes_sc_raw = st.session_state.scores_villes_cumul
            # Prépare un dict avec admins en int pour l'affichage
            villes_sc = {}
            for v, d in villes_sc_raw.items():
                villes_sc[v] = {
                    "score":   d["score"],
                    "nb":      d["nb"],
            ...
            if not top5_v:
                st.info("Aucune ville active pour l'instant. Le classement se remplira dès la première alerte soumise.")
                return
