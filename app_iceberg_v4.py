...
def page_predictions_ml(df: pd.DataFrame):
    ...
    with tab_overview:
        ...
        with c_left:
            ...
            chart_sc = alt.Chart(df_scatter).mark_circle(size=55, opacity=0.7).encode(
                x=alt.X("score_attractivite:Q", title="Score 2025", scale=alt.Scale(domain=[0,1]),
                        axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8", labelFontSize=10)),
                y=alt.Y("pred_attractivite_2026:Q", title="Prédiction 2026", scale=alt.Scale(domain=[0,1]),
                        axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8", labelFontSize=10)),
                color=alt.Color("tendance:N", scale=alt.Scale(
            ...
            ).properties(height=280, title=alt.TitleParams("Attractivité 2025 vs Prédiction 2026",
                fontSize=13, fontWeight=700, color="#0A0F1E")
            ).configure_view(strokeWidth=0).configure_axis(labelFont="Plus Jakarta Sans", titleFont="Plus Jakarta Sans")

            # Ligne diagonale y=x
            diag = alt.Chart(pd.DataFrame({"x":[0,1],"y":[0,1]})).mark_line(
                color="#CBD5E1", strokeDash=[4,4], strokeWidth=1.5
            ).encode(x="x:Q", y="y:Q")
            st.altair_chart((diag+chart_sc), use_container_width=True)
    ...
    with tab_timeline:
        ...
        chart_tl = (line_hist + area_pred + line_pred).properties(
            height=280,
            title=alt.TitleParams(
                f"{ville_tl} · {'Score attractivité' if indicateur_tl=='attractivite' else indicateur_tl.title()} 2019–2026",
                fontSize=13, fontWeight=700, color="#0A0F1E"
            )
        ).configure_view(strokeWidth=0).configure_axis(labelFont="Plus Jakarta Sans", titleFont="Plus Jakarta Sans")
        ...
        if villes_comp:
            ...
            chart_multi = alt.Chart(df_multi).mark_line(
                point=alt.OverlayMarkDef(filled=True, size=40), strokeWidth=2
            ).encode(
                x=alt.X("annee:O", title="Année",
                        axis=alt.Axis(labelColor="#8895AA", gridColor="#F0F4F8", labelFontSize=10)),
                y=alt.Y(f"{indicateur_tl}:Q", title="",
